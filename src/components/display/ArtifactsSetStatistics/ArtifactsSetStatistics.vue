<template>
    <div>
        <el-divider>Effective Stats distribution</el-divider>
        ...
        >
            ...
            <el-table-column
                label="Minimum Enhancement Number"
            >
                <template slot-scope="scope">
                    {{ scope.row.value[0] }}
                </template>
            ...
            <el-table-column
                label="Maximum Number of Enhancements"
            >
                <template slot-scope="scope">
                    {{ scope.row.value[1] }}
                </template>
        ...
        <p class="single-item">Total number of valid entries (theoretical maximum: 45).{{ totalEff.toFixed(3) }}</p>
        <!-- <p class="single-item">总有效词条数（理论最大值：45）：{{ totalEff.toFixed(3) }}</p> -->
        <p class="single-item">Crit rate + Crit damage effective entry count：{{ validEff(["critical", "criticalDamage"]).toFixed(3) }}</p>
        <p class="single-item">Crit rate + Crit damage + ATK% effective entry count：{{ validEff(["critical", "criticalDamage", "attackPercentage"]).toFixed(3) }}</p>
    </div>
</template>
